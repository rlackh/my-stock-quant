...
def get_market_top_trades():
    pool = [
        {"name": "SK하이닉스", "f_vol": 1250400, "i_vol": 842000},
        {"name": "삼성전자", "f_vol": 980300, "i_vol": 610500},
        {"name": "HD현대일렉트릭", "f_vol": 452100, "i_vol": 321000},
        {"name": "알테오젠", "f_vol": 310200, "i_vol": 215000},
        {"name": "현대차", "f_vol": 289000, "i_vol": 194000},
        {"name": "두산에너빌리티", "f_vol": -410500, "i_vol": -312000},
        {"name": "한화에어로스페이스", "f_vol": -290100, "i_vol": -201000},
        {"name": "KB금융", "f_vol": -185000, "i_vol": -142000},
        {"name": "기아", "f_vol": -142000, "i_vol": -98000},
        {"name": "NAVER", "f_vol": -110500, "i_vol": -75000}
    ]
    b_list, s_list = [], []
    for i in range(5):
        r = pool[i]
...
if ticker_code:
    ...
    if df.empty or len(df) < 5:
        ...
        # =========================================================================
        # 10. [핵심] 4대 원칙 융합 5대 심층 리서치 프롬프트 자동 생성기 (탭 UI)
        # =========================================================================
        st.markdown("### 🤖 수석 애널리스트 5대 심층 리서치 프롬프트 생성기")
        st.caption("※ 회원님의 4대 분석 원칙이 자동 결합된 5가지 롤플레잉 프롬프트입니다. 원하는 탭을 선택하여 복사 후 AI 질문창에 입력하십시오.")
        
        col_p1, col_p2, col_p3 = st.columns(3)
        compare_name = col_p1.text_input("📊 비교 대상 종목", "SK하이닉스")
        target_sector = col_p2.text_input("🌐 관심 섹터", "반도체/AI")
        target_theme = col_p3.text_input("🚀 주도 테마", "AI 및 전력 인프라")
        
        held_stock = st.text_input("💼 보유 포트폴리오 종목", search_name)
...
---"""
        ...
        with tab1:
            news_title = st.text_input("뉴스 제목/핵심 요약 입력", placeholder="예: 2분기 사상 최대 실적 달성 및 HBM 공급 확대")
            p1 = f"{master_prompt}\n\n너는 냉철한 주식 시장 분석가야. 방금 나온 '{search_name}'의 뉴스 [{news_title or '제목/내용 요약'}]을 분석해 줘. 이 뉴스가 단기 및 중장기적으로 주가에 긍정적인지 부정적인지 판단하고, 그 핵심 이유를 3가지로 명확히 요약해 줘. 마지막으로 이 뉴스를 해석할 때 개인 투자자가 흔히 범할 수 있는 오류나 주의해야 할 리스크도 함께 짚어줘."
